...
public class APlayerEntedMessage implements Message {
	
	int msgType=Message.A_PLAYER_ENTED_MSG;
	Paticipator paticipator;
	int roomId;
	
	public APlayerEntedMessage(Paticipator paticipator) {
		this.paticipator=paticipator;
	}
	
	public APlayerEntedMessage(Paticipator paticipator, int roomId) {
		super();
		this.paticipator = paticipator;
		this.roomId = roomId;
	}
	...
	public void parse(DataInputStream dis) {
		try {
			...
			if(paticipator.getMyId()!=myId) {
				System.out.println("player "+name+" into this room");
			}
